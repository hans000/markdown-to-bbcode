...
export function colorProvider(monaco: any) {
    monaco.languages.registerColorProvider('markdown', {
        provideColorPresentations: (model: any, colorInfo: any) => {
            ...
            var a = Math.round(color.alpha * 255)
            var label = rgb2hex({ r, g, b, a })
        ...
        provideDocumentColors: (model: any) => {
            ...
            let hasStart = false
            model.getLinesContent().forEach((line: string, index: number) => {
                if (line.startsWith('#region')) {
                    const reg = /(?:color|backcolor|tablerow|tr)=(.+?)(?=\s|$)/g
                    while (true) {
                        const match = reg.exec(line)
                        if (! match) {
                            break
                        }
                        const colorNameOrHex = match[1]
                        let colorObj = colorMap[colorNameOrHex]
                        if (! colorObj) {
                            colorObj = hex2rgb(colorNameOrHex)
                        } else {
                            colorObj.a = 255
                        }
                        if (colorObj) {
                            colorInfos.push({
                                color: {
                                    red: (colorObj.r / 255),
                                    green: (colorObj.g / 255),
                                    blue: (colorObj.b / 255),
                                    alpha: (colorObj.a / 255),
                                },
                                range: {
                                    startLineNumber: index + 1,
                                    endLineNumber: index + 1,
                                    startColumn: reg.lastIndex - colorNameOrHex.length + 1,
                                    endColumn: reg.lastIndex + 1
                                }
                            })
                        }
                    }
                ...
                if (hasStart) {
                    const reg = /(?:color|backcolor|tablerow|tr)\s*:\s*"(.+?)(?=")/g
                    while (true) {
                        const match = reg.exec(line)
                        if (! match) {
                            break
